...
Afin de controller le servomoteur, nos devons envoyer des impulsions electrique toutes les 1 à 2ms avec une période de 20ms entre chaque front se qui est largement en dessous des performance de la fonction `osDelay` de ce fait, nous avons utilisé une des horloges internes de la carte STM32 et d'un timer. 

Nous avons choisi de réglé le timer sur une periode de `20000ns` ns avec un prescaler de `168` (168 étant la vitesse en Khz de l'horloge) afin de nous ramener à une unité de 1ms

Ensuite nous réglons le `Pulse` (valeur de trigger de l'interrupt du timer) à `1000ns` afin qu'une impulsion soit envoyé après `1ms`


Exemple d'abstraction possible pour la gestion du servomoteur (fichier servo.c)
```c
// Demande le servo moteur à aller à un angle précis (entre 0 et 90)
void set_angle(TIM_OC_InitTypeDef* sConfigOC, int angle) {
	  HAL_TIM_PWM_Stop(&htim1, TIM_CHANNEL_1);

	  sConfigOC->Pulse = angle * 1000 / 90 + 1000;
