...
from local_planner_student.srv import Path as PathToGoal, PathRequest
...
class TravelState(Enum):
    ...
    PENDING = 1
    REACHED_DISTANCE = 2
    REACHED_DISTANCE_ORIENTATION = 3
...
class LocalPlanner:
    ...
    def __init__(self, K_linear, K_angular, Sat_linear, Sat_angular, Obstacle_range, Angle_to_allow_linear, Waypoint_error, Destination_error, Angle_error):
        # init params
        self.K_linear = K_linear  # Proportional coefficient for linear velocity
        self.K_angular = K_angular  # Proportional coefficient for angular velocity
        ...
        self.Waypoint_error = Waypoint_error  # Euclidean distance error to a waypoint allowing to move to a new waypoint
        self.Destination_error = Destination_error  # Euclidean distance error to the final waypoint below which we consider the position reached
    ...
    def pathService(self, req: PathRequest):
        """
    ...
    def computeDistAngle(self):
        ...
        if len(self.pathPoses) > 0:
            ...
            angle = self.shortestAngleDiff(steering, self.curPose2D.theta)
    ...
    def pathSequencer(self, dist, angle, finalOrientation) -> TravelState:
        """
