...
class ShortPathMng:
    ...
    def __init__(self, resolution, shortPathMethod, isLocalPlanner, inflate_radius):
        ...
        if isLocalPlanner:
            self.pathGoalService = rospy.ServiceProxy('/move_to/pathGoal', PathToGoal)
            self.pathGoalService.wait_for_service()
    ...
    def inflate_map(self, map, map_resolution):
        ...
        nb_size_to_add = int(math.ceil(self.inflate_radius / (map_resolution * self.RESOLUTION)))

        obstacles = []

        for x, col in enumerate(map):
            for y, row in enumerate(col):
                if row == self.MAP_OBSTACLE_VALUE:
                    obstacles.append({'x': x, "y": y})

        for obstacle in obstacles:
            left_x = obstacle["x"] - nb_size_to_add
            left_y = obstacle["y"] - nb_size_to_add
            for i in range(0, 2 * nb_size_to_add + 1):
                for j in range(0, 2 * nb_size_to_add + 1):
                    cell_x = left_x + i
                    cell_y = left_y + j
    ...
    def pushGoals(self, mapNode, start, marker_container, isreverted, isPathOnService):
        ...
        if isPathOnService:
            ...
            pathRequest.pathToGoal.poses = list(goalQueue.queue)
            self.pathGoalService.call(pathRequest)
