...
class AStar(AbstractShortPath):
    ...
    def goto(self, source, target, matrix, pub_marker, marker_container):
        ...
        while len(frontier) != 0 and not isEnd:
            ...
            current_heuristic = self.get_heuristic(current, target)
            print(f"Min is {current_str}: score={fscore[current_str]} matrix={matrix[current['y']][current['x']]}")
            ...
            for neighbor in neighbors:
                neighbor_str = self.cell_str(neighbor)
                ...
                if self.isNotProcessed(neighbor, frontier):
                    ...
                    #fscore[neighbor_str] = fscore[current_str] + matrix[neighbor["y"]][neighbor["x"]] + 1 + self.get_heuristic(neighbor, target) - current_heuristic
                    gnscore = fscore[current_str] + 1 - current_heuristic
                    fscore[neighbor_str] = gnscore + self.get_heuristic(neighbor, target)

                    # fscore[neighbor_str] = self.get_heuristic(matrix, neighbors, target)
                    ...
                    prev[neighbor_str] = current_str
                    # check if the current neighbor is the target
                    if neighbor_str == self.cell_str(target):
    ...
    def minScore(self, fscore, frontier):
        ...
        min_coord = {'x': -1, 'y': -1}
        for n in frontier:
            if fscore[self.cell_str(n)] < score_min:
                score_min = fscore[self.cell_str(n)]
                min_coord = n
        return min_coord
