...
class Tester(object):
    def __init__(self):
        ...
        # convert into [rad/s]
        if self.debug: print
        self.command_vx, self.command_wz
        ...
        if self.debug: print
        'init time: {0:2.4f}'.format(self.init_time)
    ...
    def timerCallback(self, event):
        ...
        if self.state == "ALIGNING":
            ...
            if abs(scan_angle) > radians(1.0):
                ...
                self.init_time = rospy.Time.now().to_time()
                if self.debug: print
                'state:', self.state, ':', scan_angle, '-->', cmd_vel.angular.z
                return
            else:
                cmd_vel = Twist()
        ...
        if self.state == 'CALC_ANGLE_PRERUN':
            if self.angle_count >= self.max_sample:
                ...
                if self.debug: print
                'angle prerun: ', self.angle_prerun
                ...
                if self.debug: print
                'state:', self.state, ':', self.angle_count, self.scan_angle, self.angle_sum, self.angle_avg
                return
        ...
        if self.state == 'RUNNING':
            if self.ending_condition():
                ...
                self.pub_velocity.publish(cmd_vel)
                if self.debug: print
                'state:', self.state, ':', degrees(self.accumulated_angle), self.test_angle
                return
...
if __name__ == '__main__':
    print
    print
    "It test gyro on kobuki and print gyro drift errors."
    print
    try:
        instance = Tester()
        rospy.spin()
