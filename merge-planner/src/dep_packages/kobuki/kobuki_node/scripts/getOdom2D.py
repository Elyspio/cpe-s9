...
roslib.load_manifest('kobuki_node')
import rospy
...
class Converter(object):
    ...
    def OdomCallback(self, data):
        ...
        print
        "pose: x: {0:+2.5f}".format(px) + ", y: {0:+2.5f}".format(py) \
        + ", th: {0:+.4f}".format(yaw) + " rad; " \
        + "{0:+.2f}".format(degrees(yaw)) + " deg"
        print
        "rate: x: {0:+2.5f}".format(vx) + ", y: {0:+2.5f}".format(vy) \
...
if __name__ == '__main__':
    try:
        ...
        print
        print
        "It prints x, y, theta values from Odom message of mobile base."
        print
        rospy.spin()
